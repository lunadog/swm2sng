 ...
 * Inverse of sng2swm.cpp
 *
 * Build: g++ -std=c++17 -O2 -s swm2sng.cpp -o swm2sng
 *
 * Pattern-block parsing fix (the bug in the original):
 * -------------------------------------------------------
 * The original code tried to find the pattern SIZE byte by
 * checking if the byte value equals its 1-based position in
 * the block (self-referential size).  That only works for
 * patterns without NOP-packing.  For packed patterns the
 * stored SIZE value (unpacked byte count) is larger than the
 * packed position, so the check never fires or fires on a
 * spurious coincidence.
 *
 * The correct algorithm uses the LENGTH byte instead:
 *   - Parse rows one at a time.
 *   - After every complete row, peek one byte ahead of the
 *     current file position.
 *   - If that peek byte equals the number of rows parsed so
 *     far, the CURRENT byte is the SIZE byte and the PEEK byte
 *     is the LENGTH byte.  End of block.
 *
 * Row encoding in the packed stream (what SW1_PTNCOL1_PACKMIN/
 * PACKMAX actually mean in real SID-Wizard files):
 *   0x70..0x77  at the start of a row position = a run of
 *               (byte - 0x70 + 2) rest rows, no instrument byte.
 *   0x7E        = one rest row, no instrument byte.
 *   0x00..0x6F, 0x78..0x7D, 0x7F
 *               = other note-only rows (no instrument byte),
 *                 including 0x00 which sng2swm uses as a rest.
 *   0x80..0xFF  = note row WITH instrument byte following.
 *     instrument byte bit7=1 -> FX byte follows.
 *       FX < SW1_SMALLFX_MIN -> parameter byte follows (big FX).
 *       FX >= SW1_SMALLFX_MIN -> no parameter byte (small FX).
 *
 * Sequence format (unchanged from original, confirmed correct):
 *   [pattern-ref bytes ...] [0xFF loop-marker] [restart-pos byte] [size byte]
 *
 * Instrument table format (unchanged, confirmed correct):
 ...
 * Globals
 *************************************************************/
static sng_table gt2_wavetable;
static sng_table gt2_pulsetable;
static sng_table gt2_filtertable;
static sng_table gt2_speedtable;
...
static std::vector<uint8_t> UnpackPattern(const std::vector<uint8_t>& packed) {
    ...
    for (size_t i = 0; i < packed.size(); ) {
        ...
        if (b >= 0x70 && b <= 0x77) {
             ...
             * 0x7E = SW1_PTNCOL1_KEYOFF (gate off) — distinct from rest. */
            int count = (b - 0x70) + 2;
...
static void ConvertPattern(const SW_Pattern& sw, sng_pattern& gt,
    ...
    while (idx < unpacked.size() && row < gt.rows - 1) {
         ...
         *   0x00        = SW1_PTNCOL1_REST   — rest (gate stays on, note continues)
         *   0x7E        = SW1_PTNCOL1_KEYOFF — explicit gate-off (release envelope)
         *   0x70..0x77  = rest run (already expanded to 0x00 bytes by UnpackPattern)
         *   0x80+       = note with instrument byte
         *   0x01..0x6F  = plain note (no instrument byte)
         *
         * 0x7E is SW1_PTNCOL1_KEYOFF, NOT a rest.  The pack range 0x70-0x77 encodes
         * runs of plain rests (0x00), which UnpackPattern has already expanded. */
        if (note == 0x00) {
...
void Convert(const SWMFile& swm, GT2File& gt) {
    ...
    for (int i = 0; i < swm.header.instrument_count; i++) {
        ...
        if (!instr_has_filter(i) || __builtin_popcount(vmask) <= 1) {
